...
const presignSchema = z.object({
  ...
  purpose: z.enum(["original", "preview", "attachment"]),
});
...
uploadsRouter.post("/presign", async (req, res) => {
  ...
  const key = `uploads/${projectId}/${purpose}/${stamp}-${safe}`;
  const url = await presignUpload(key, contentType);
  res.json({ key, url });
});

const downloadSchema = z.object({
  key: z.string().regex(/^uploads\/[0-9a-f-]{36}\//i, "Invalid key"),
...
uploadsRouter.get("/download", async (req, res) => {
  ...
  const projectId = parsed.data.key.split("/")[1]!;
  if (!(await canAccessProject(req, projectId))) {
